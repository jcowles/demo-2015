...
void main( void )
{
    ...
    if (uvCoord.y < 1 && uvCoord.y > 0) {
        col = texture(iChannel1, uvCoord);
        //col.rgb = Fxaa(posPos, iChannel1, 1/iResolution.xy);
    }
    ...
    //if (uvCoord.y < 1 && uvCoord.y > 0)
    //    col.rgb = clamp(col.rgb + grain, vec3(0,0,0), vec3(1,1,1));
	color = col;
}
